...
function build_matcher_object(pairMatchers) {
    return function(what) {
        ...
        for (var i = 0; i < pairMatchers.length; ++i) {
            ...
            capAcc = capAcc.concat(testRes.captures);
        }
...
function deep_continue_until_value_match(what, matcher) {
    var fun = isObject(what) ?
...
function deep_continue_until_value_match_object(what, matcher) {
    for (var k in what) {
        var value = what[k];
        var res = matcher(value);
        if (res.status) {
            return res; // <== 
        }
        else {
            ...
            var deepRes = deep_continue_until_value_match(value, matcher);
            if (deepRes.status) {
                return deepRes; // <== 
            }
            else {
...
function deep_continue_until_value_match_list(what, matcher) {
    for (var k = 0; k < what.length; ++k) {
        var value = what[k];
        var res = matcher(value);
        if (res.status) {
...
function build_matcher_capture(name, matcher) {
    return function(what) {
        var res = matcher(what);
        if (res.status) {
            var cap = {};
            cap[name] = what;
            return build_matching_result(true, [cap].concat(res.captures)); // <== 
        }
        else {
            return res;
        }
